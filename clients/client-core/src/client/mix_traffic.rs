...
pub type BatchRealMessageSenderForNetwork = mpsc::UnboundedSender<Vec<MixPacket>>;
pub type BatchRealMessageReceiverForNetwork = mpsc::UnboundedReceiver<Vec<MixPacket>>;
...
impl MixTrafficController {
    ...
    async fn on_real_messages(&mut self, mut mix_packets: Vec<MixPacket>) {
        debug_assert!(!mix_packets.is_empty());
        
        let result = if mix_packets.len() == 1 {
            let mix_packet = mix_packets.pop().unwrap();
            self.gateway_client.send_real_mix_packet(mix_packet).await
        } else {
            self.gateway_client
                .batch_send_real_mix_packets(mix_packets)
                .await
        };
